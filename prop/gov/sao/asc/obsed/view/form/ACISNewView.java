...
/******************************************************************************/
/**
 * Shows the ACIS table plus in a form view.
 *
...
public class ACISNewView extends NewView 
{
  /****************************************************************************/
  /**
   * Construct a view which will be used to create a new ACIS record.
   */
  ...
  {
    System.out.println( "Inserting a new record into the acisparam table." );

    // Get the number of existing rows in this table.
    int rowCount = model.getRowCount( "acisparam" );

    try
    {
      // Insert a new record, or get the row number for the matching entry.
      int newRecordNumber = database.createNewACISParamRecord( newKeyValue );

      // Assert: newRecordNumber != -1.
      if ( newRecordNumber <= rowCount )
      {
	JOptionPane.showMessageDialog( this,
				       "This ACIS parameter set already " + 
				       "exists as ACIS ID " + 
				       newRecordNumber + ".",
    ...
    {
      String message = "ACIS Param table insertion failed.";
