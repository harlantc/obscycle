...
#include "utlib.h"
void heapsort_r( const  float* x, const long n, ArrayIndex* jsort );
void shellsort_r( const float* x, const long n, ArrayIndex * jsort );

static void sort_equals( const float* x, const long n, long* jsort )
{
...
void utn_ar_sort_r( const float* x, const long n, long* jsort )
{
 if ( n > UT_PREFER_HEAPSORT )
  heapsort_r( x, n, jsort );
 else if ( n <= 1 ) 
  jsort[0] = 1;
 else
  shellsort_r( x, n, jsort );
}
...
void utn_ar_resort_i( const long* x, const long n, long* y, long* jsort )
{
 long i;
 for ( i = 0; i < n; i++ ) {
  y[i] = x[jsort[i]-1];
