...
static void utn_sort_equals( const float* x, const long n, long* jsort )
{
...
void utn_cheapsort_r( const float* x, const long n, ArrayIndex* jsort )
{
 float y;
 logical loop;
 long index;
...
void utn_cshellsort_r( const float* x, const long n, ArrayIndex * jsort )
{
 long log2n;
 ArrayOffset i;
 logical loop;
...
void utn_ar_csort_r( const float* x, const long n, long* jsort )
{
 if ( n > UT_PREFER_HEAPSORT )
  utn_cheapsort_r( x, n, jsort );
 else if ( n <= 1 ) 
  jsort[0] = 0;
 else
  utn_cshellsort_r( x, n, jsort );
}
