...
static void sort_equals( const short* x, const long n, long* jsort )
{
...
void utn_cheapsort_s( const short* x, const long n, ArrayIndex* jsort )
{
 short y;
 logical loop;
 long index;
...
void utn_cshellsort_s( const short* x, const long n, ArrayIndex * jsort )
{
 long log2n;
 ArrayOffset i;
 logical loop;
...
void utn_ar_csort_s( const short* x, const long n, long* jsort )
{
 if ( n > UT_PREFER_HEAPSORT )
  utn_cheapsort_s( x, n, jsort );
 else if ( n <= 1 ) 
  jsort[0] = 0;
 else
  utn_cshellsort_s( x, n, jsort );
}
