...
void utn_heapsort_d( const double* x, const long n, ArrayIndex* jsort )
{
 double y;
 logical loop;
 long index;
...
void utn_shellsort_d( const double* x, const long n, ArrayIndex * jsort )
{
 long log2n;
 ArrayOffset i;
 logical loop;
...
void utn_ar_sort_d( const double* x, const long n, long* jsort )
{
 if ( n > UT_PREFER_HEAPSORT )
  utn_heapsort_d( x, n, jsort );
 else if ( n <= 1 ) 
  jsort[0] = 1;
 else
  utn_shellsort_d( x, n, jsort );
}
