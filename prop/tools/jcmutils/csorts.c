...
static void utn_sort_equals( const short* x, const long n, long* jsort )
{
...
void utn_heapsort_s( const short* x, const long n, ArrayIndex* jsort )
{
 short y;
 logical loop;
 long index;
...
void utn_shellsort_s( const short* x, const long n, ArrayIndex * jsort )
{
 long log2n;
 ArrayOffset i;
 logical loop;
...
void utn_ar_sort_s( const short* x, const long n, long* jsort )
{
 if ( n > UT_PREFER_HEAPSORT )
  utn_heapsort_s( x, n, jsort );
 else if ( n <= 1 ) 
  jsort[0] = 1;
 else
  utn_shellsort_s( x, n, jsort );
}
