...
*                   operational units.
*       Modified in 2011 Apr to include NuSTAR and Hitomi SXS
*       Last modified in 2019 Aug to include Athena
...
*         Hard-wired code for Hitomi SXS (1101: open, 1102: Be25 filter;
...
*         Hard-wired call for Hitomi SGD
          if( restr ) then
            call PWRITE( '% PIMMS provides Hitomi SGD background ' //
...
*         Hard-wired code for MAXI GSC and SSC
     ...
     &                                                    'c/s/cm^2)' )
        ...
        else if( special .eq. 1301 ) then
*         This hard-wired code means we're dealing with NICER
          if( restr ) then
            call PWRITE( '% Further instrument-specific information ' //
     &                      'can be obtained by re-running PIMMS' )
            call PWRITE( '  without specifying a limited ' //
     &                                    '(non-default) energy range' )
            ...
            call NICER_LIMIT( results, n_res )
          end if

        else if ( special .ge. 1401 .and. special .le. 1404 ) then
*         Hard-wired code for Athena
          call PWRITE( '% Count rate estimated ' //
