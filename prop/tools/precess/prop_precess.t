...
# !!3
toolname="prop_precess"
toolE="prop_precess_exe"
...
do
  ...
  case ${testid} in
    # !!6
    test1 ) test1_string="${toolname} P2 f b t g :${INDIR}/bfile > ${outfile}"
            echo $test1_string | tee -a  $LOGFILE 
            echo "" | tee -a  $LOGFILE
	    eval $test1_string
            ;;
    test2 ) test1_string="${toolname} P2 f b t j :${INDIR}/bfile > ${outfile}"
            echo $test1_string | tee -a  $LOGFILE 
            echo "" | tee -a  $LOGFILE
	    eval $test1_string
            ;;
    test3 ) test1_string="${toolname} P2 f b t sg :${INDIR}/bfile > ${outfile}"
            echo $test1_string | tee -a  $LOGFILE 
            echo "" | tee -a  $LOGFILE
	    eval $test1_string
            ;;
    test4 ) test1_string="${toolname} P2 f b t EC :${INDIR}/bfile > ${outfile}"
            echo $test1_string | tee -a  $LOGFILE 
            echo "" | tee -a  $LOGFILE
	    eval $test1_string
            ;;
    test5 ) test1_string="${toolname} P2 f b t CON :${INDIR}/bfile > ${outfile}"
            echo $test1_string | tee -a  $LOGFILE 
            echo "" | tee -a  $LOGFILE
	    eval $test1_string
            ;;
    test6 ) test1_string="${toolname} P2 f g t g e 95.5570  -8.3 > ${outfile}"
            echo $test1_string | tee -a  $LOGFILE 
            echo "" | tee -a  $LOGFILE
	    eval $test1_string
            ;;
  ...
  case ${testid} in
    test1|test3 ) cat  ${outfile} | grep -i galactic > ${outstd}
            cat  ${savfile} | grep -i galactic > ${savstd}
            ;;
    test2 ) cat ${outfile} | grep J2000 > ${outstd}
            cat ${savfile} | grep J2000  > ${savstd}
            ;;
    test4 ) cat ${outfile} | grep Ecliptic > ${outstd}
            cat ${savfile} | grep Ecliptic > ${savstd}
            ;;
    test5 ) cat ${outfile} | grep Constell > ${outstd}
            cat ${savfile} | grep Constell > ${savstd}
            ;;
    test6 ) cat ${outfile} | grep Galactic > ${outstd}
            cat ${savfile} | grep Galactic > ${savstd}
            ;;
  esac
